...
const AI_OBJECTS = [
  ...
  createAIObject("t1", "Matt konservburk", "metal", "testing", {
    type: "can",
    variant: "mid",
    accent: "steel",
  ...
  }, "En matt och rund konservburk som inte fanns i träningen."),
  createAIObject("t2", "Blank plastförpackning", "plastic", "testing", {
    type: "container",
    variant: "tray",
    accent: "violet",
    detail: "clear-window",
  ...
  }, "En blank plastförpackning med rundade hörn."),
  createAIObject("t3", "Målad metallkapsyl", "metal", "testing", {
    type: "cap",
    variant: "crimped",
    accent: "blue",
    detail: "center-dot",
  ...
  }, "En blåmålad, rund metallkapsyl."),
  createAIObject("t4", "Genomskinlig hård plastlåda", "plastic", "testing", {
    type: "crate",
    variant: "open-top",
    accent: "clear",
    detail: "grid",
  ...
  }, "En genomskinlig och kantig hård plastlåda."),
...
function sanitizeAILab(value) {
  ...
  clean.trained = Boolean(value.trained) && Object.keys(clean.labels).length === 6;
  clean.installed = Boolean(value.installed) && clean.trained;
  clean.completed = Boolean(value.completed) && clean.installed;
  clean.hasSeenIntro = Boolean(value.hasSeenIntro);
  const sortingCount = clean.completed
    ? getAIObjectsByGroup("sorting").length
    : clampNumber(value.sortingIndex, 0, getAIObjectsByGroup("sorting").length - 1, 0);
  ...
  if (clean.trained) clean.testResults = evaluateAIModel(clean.labels);
  if (clean.installed) {
...
function renderLevelSelection() {
  ...
  aiCard.innerHTML = `
    <span class="ai-chapter-icon" aria-hidden="true">${aiUnlocked ? "AI" : "🔒"}</span>
...
async function executeDrop(runId, delay) {
  ...
  if (!target || !samePosition(target, level.delivery)) {
    return { type: "failure", message: "Här är ingen leveransplats." };
  }
      ...
      <section class="ai-panel" aria-labelledby="training-cards-title">
        <div class="ai-panel-heading">
          <div>
            ...
            <p>Välj vad AI:n ska få lära sig. Facit är dolt medan du tränar.</p>
          </div>
...
function renderAITestStage() {
  const results = state.aiLab.testResults.length ? state.aiLab.testResults : evaluateAIModel();
  const score = results.filter((result) => result.correct).length;
  return `
    <section class="ai-panel" aria-labelledby="ai-test-title">
      <div class="ai-panel-heading">
        <div>
          <p class="card-kicker">Test med nya föremål</p>
          <h2 id="ai-test-title">AI-kameran fick ${score} av 4 rätt</h2>
          <p>Dessa föremål fanns inte bland träningskorten. Jämför AI:ns bedömning med facit.</p>
        </div>
        <span class="ai-count">${score} / 4 rätt</span>
      </div>
      <div class="ai-test-grid">
        ${results.map(renderAITestResult).join("")}
      </div>
      <div class="ai-test-actions">
        <button class="secondary-button" type="button" data-ai-action="improve">Förbättra träningsdata</button>
        <button class="primary-button" type="button" data-ai-action="install">Installera modellen i roboten</button>
      </div>
...
function renderAITestResult(result) {
  return `
    <article class="ai-test-card ${result.correct ? "is-correct" : "is-incorrect"}">
      ${renderAIObjectVisual(result.item, { decorative: true })}
      <strong>${result.item.name}</strong>
      <div class="ai-test-result">
        <span>AI:n sa: <strong>${formatAICategory(result.predictedLabel)}</strong></span>
        <span>Facit: <strong>${formatAICategory(result.item.trueCategory)}</strong></span>
        <span class="ai-result-mark ${result.correct ? "is-correct" : "is-incorrect"}">${result.correct ? "Rätt bedömning" : "Fel bedömning"}</span>
      </div>
    </article>
  `;
}
...
function renderAISortingStage() {
  ...
  if (!current) return renderAICompleteStage();
  return `
...
function labelAITrainingObject(id, label) {
  ...
  saveProgress();
  renderAILab();
  showAIStatus(`${getAIObject(id).name} märktes som ${formatAICategory(label)}.`, "info");
}
...
function removeAITrainingObject(id) {
  ...
  saveProgress();
  renderAILab();
  showAIStatus("Kortet togs bort från träningssamlingen.", "info");
}

function invalidateAIModel() {
...
async function trainAIModel() {
  ...
  state.aiLab.testResults = evaluateAIModel();
  state.aiLab.sortingIndex = 0;
  state.aiLab.sortingResults = [];
  saveProgress();
  renderAILab();
...
function predictAIObject(item, labels = state.aiLab.labels) {
  ...
  if (!examples.length) return { label: "metal", confidence: 0, neighbors: [] };
...
function evaluateAIModel(labels = state.aiLab.labels) {
  return getAIObjectsByGroup("testing").map((item) => {
    const prediction = predictAIObject(item, labels);
    return {
      item,
      predictedLabel: prediction.label,
      correct: prediction.label === item.trueCategory,
...
function returnToAITraining() {
  ...
  saveProgress();
  renderAILab();
  showAIStatus("Byt, ta bort eller märk om träningskort och träna modellen igen.", "info");
}

function installAIModel() {
  if (!state.aiLab.trained) return;
...
async function runAISortingStep() {
  ...
  if (state.aiLab.sortingIndex >= getAIObjectsByGroup("sorting").length) {
    state.aiLab.completed = true;
    state.aiLab.stage = "complete";
  }
  saveProgress();
  renderAILab();
  showAIStatus(
    state.aiLab.completed
      ? "Sorteringsuppdraget är klart."
